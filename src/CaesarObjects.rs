...
impl CodecFunctions for CaesarEncoder
{
    fn encode(data: &Vec<u8>, key: Option<&i64>) -> std::io::Result<Vec<u8>>
    {
        ...
        let key_ref: &i64 = key.expect("Error: No key");
        let key_bytes: [u8; 8] = key_ref.to_be_bytes();
    ...
    fn decode(encoded_data: &Vec<u8>, possible_key: Option<&i64>) -> std::io::Result<Vec<u8>>
    { 
        if encoded_data.len() < 33 { return Ok(Vec::new()); }

        let hashed_key = &encoded_data[..32];

        let possible_key_ref: &i64 = possible_key.expect("Error: No key");
        let possible_key_bytes: [u8; 8] = possible_key_ref.to_be_bytes();
